...
Video_Stabilization_Using_Point_Feature_Matching
...
% This stabilization algorithm involves two steps. First, we determine the affine image transformations between all neighboring frames of a video sequence using the estimateGeometricTransform function applied to point correspondences between two images. Second, we warp the video frames to achieve a stabilized video. We will use the Computer Vision Toolbox™, both for the algorithm and for display.
% This example is similar to the Video Stabilization Example. The main difference is that the Video Stabilization Example is given a region to track while this example is given no such knowledge. Both examples use the same video.
...
[a_1 a_3 0;
 a_2 a_4 0;
 t_x t_y 1]
% The parameters  define scale, rotation, and shearing effects of the transform, while the parameters  are translation parameters. This transform can be used to warp the images such that their corresponding features will be moved to the same image location.
...
% Given a set of video frames , we can now use the above procedure to estimate the distortion between all frames  and  as affine transforms, . Thus the cumulative distortion of a frame  relative to the first frame will be the product of all the preceding inter-frame transforms, or
...
[s*cos(ang)  s*-sin(ang)  0;
 s*sin(ang)   s*cos(ang)  0;
        t_x         t_y   1]
% We show this conversion procedure below by fitting the above-obtained transform  with a scale-rotation-translation equivalent, . To show that the error of converting the transform is minimal, we reproject frame B with both transforms and show the two images below as a red-cyan color composite. As the image appears black and white, obviously the pixel-wise difference between the different reprojections is negligible.
...
% At each step we calculate the transform  between the present frames. We fit this as an s-R-t transform, . Then we combine this the cumulative transform, , which describes all camera motion since the first frame. The last two frames of the smoothed video are shown in a Video Player as a red-cyan composite.
